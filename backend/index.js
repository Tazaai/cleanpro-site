...
// ✅ Ensure Firebase key file exists (using FIREBASE_KEY from GitHub secret)
const SERVICE_ACCOUNT_PATH = "/app/backend/serviceAccountKey.json";
try {
  if (!existsSync(SERVICE_ACCOUNT_PATH)) {
    if (!process.env.FIREBASE_KEY) {
      throw new Error("Missing FIREBASE_KEY in environment");
    }
  ...
  console.error("⚠️ Could not create serviceAccountKey.json:", err.message);
}
